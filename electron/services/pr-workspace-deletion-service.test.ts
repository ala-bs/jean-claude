...
function makeDeps(tasks = [makeTask()]): PrWorkspaceDeletionDeps {
  ...
  return {
    ...
    keepPrWorkspaces: vi.fn(async (taskIds: string[]) =>
      taskIds.map((taskId) => {
        const updated = makeTask({
          ...
          status:
            current.get(taskId)?.status === 'completed'
              ? 'waiting'
              : current.get(taskId)?.status,
          userCompleted: false,
        });
        current.set(taskId, updated);
        return updated;
      }),
    ),
...
describe('resolveClosedPrWorkspace', () => {
  it('can Keep immediately after a grouped closure transition commits', async () => {
    const active = makeTask({ prWorkspaceState: 'active' });
    const reconcileDeps = {
      findPrReviewTasksByPullRequest: vi.fn().mockResolvedValue([active]),
      revalidatePullRequestStatus: vi.fn().mockResolvedValue('completed' as const),
      markPrWorkspacesCleanupPending: vi.fn(async () => [
        makeTask({
          ...active,
          prWorkspaceState: 'cleanup-pending',
        }),
      ]),
      reactivatePrWorkspaces: vi.fn(),
      emitTaskUpsert: vi.fn(),
    };

    const pending = await reconcilePrWorkspaceState(
      { projectId: 'project-1', pullRequestId: 12 },
      reconcileDeps,
    );
    const keepDeps = makeDeps(pending);
    await resolveClosedPrWorkspace(
      { projectId: 'project-1', pullRequestId: 12, action: 'keep' },
      keepDeps,
    );

    expect(reconcileDeps.emitTaskUpsert).toHaveBeenCalledWith(pending[0]);
    expect(keepDeps.keepPrWorkspaces).toHaveBeenCalledWith([active.id]);
    expect(keepDeps.emitTaskUpsert).toHaveBeenCalledWith(
      expect.objectContaining({ prWorkspaceState: 'kept' }),
    );
  });

  it('keeps every pending workspace with no destructive side effects', async () => {
    const tasks = [
      makeTask({ id: 'running', status: 'running' }),
      makeTask({ id: 'waiting', status: 'waiting', branchName: 'review-2' }),
      makeTask({
        id: 'completed',
        status: 'completed',
        userCompleted: true,
    ...
    expect(deps.keepPrWorkspaces).toHaveBeenCalledWith([
      'running',
      'waiting',
      'completed',
    ]);
    ...
    expect(deps.deleteTasks).not.toHaveBeenCalled();
    expect(
      vi.mocked(deps.emitTaskUpsert).mock.calls.map(([task]) => ({
        id: task.id,
        status: task.status,
        userCompleted: task.userCompleted,
      })),
    ).toEqual([
      { id: 'running', status: 'running', userCompleted: false },
      { id: 'waiting', status: 'waiting', userCompleted: false },
      { id: 'completed', status: 'waiting', userCompleted: false },
    ]);
  });

  it('preserves a running agent task through closure and Keep', async () => {
    ...
    const kept = vi.mocked(deps.emitTaskUpsert).mock.calls[0][0];
    const operation = vi.fn();
    await startAgentWithPrReviewLifecycle('task-1-step', operation, {
      findStepById: vi.fn().mockResolvedValue(makeStep(kept.id)),
      findTaskById: vi.fn().mockResolvedValue(kept),
    });
    ...
    expect(operation).toHaveBeenCalledWith('task-1-step');
  });

  it('makes a migrated completed workspace usable only after Keep resolves', async () => {
    const migrated = makeTask({ status: 'completed', userCompleted: true });
    const deps = makeDeps([migrated]);
    const operation = vi.fn();

    await expect(
      startAgentWithPrReviewLifecycle('task-1-step', operation, {
        findStepById: vi.fn().mockResolvedValue(makeStep(migrated.id)),
        findTaskById: vi.fn().mockResolvedValue(migrated),
      }),
    ...
    const restored = vi.mocked(deps.emitTaskUpsert).mock.calls[0][0];
    await startAgentWithPrReviewLifecycle('task-1-step', operation, {
      findStepById: vi.fn().mockResolvedValue(makeStep(restored.id)),
  ...
  it('is idempotent after keep and delegates delete to delete-all behavior', async () => {
    const deps = makeDeps();
    await resolveClosedPrWorkspace(
      { projectId: 'project-1', pullRequestId: 12, action: 'keep' },
      deps,
    ...
    expect(deps.keepPrWorkspaces).toHaveBeenCalledTimes(1);

    await resolveClosedPrWorkspace(
      { projectId: 'project-1', pullRequestId: 12, action: 'delete' },
      deps,
    );
    expect(deps.deleteTasks).toHaveBeenCalledWith(['task-1']);
  });

  it('rejects repository results with mismatched identity', async () => {
    const deps = makeDeps();
    vi.mocked(deps.findPrReviewTasksByPullRequest).mockResolvedValue([
      makeTask({ projectId: 'other-project' }),
    ]);
    ...
    expect(deps.keepPrWorkspaces).not.toHaveBeenCalled();
  });

  it('keeps pending subset in a mixed pending and kept group', async () => {
    const deps = makeDeps([
      makeTask({ id: 'pending' }),
    ...
    await expect(
      ...
      action: 'kept',
      taskIds: ['pending', 'kept'],
    });
    expect(deps.keepPrWorkspaces).toHaveBeenCalledWith(['pending']);
    expect(deps.emitTaskUpsert).toHaveBeenCalledOnce();
    expect(deps.emitTaskUpsert).toHaveBeenCalledWith(
      expect.objectContaining({ id: 'pending', prWorkspaceState: 'kept' }),
    );
  });

  it('rejects active plus pending workspace state', async () => {
    const deps = makeDeps([
      makeTask({ id: 'pending' }),
      makeTask({ id: 'active', prWorkspaceState: 'active' }),
    ]);
    await expect(
      resolveClosedPrWorkspace(
        { projectId: 'project-1', pullRequestId: 12, action: 'keep' },
  ...
  it('rejects malformed null workspace state', async () => {
    const deps = makeDeps([makeTask({ prWorkspaceState: null })]);
    await expect(
      resolveClosedPrWorkspace(
        { projectId: 'project-1', pullRequestId: 12, action: 'keep' },
        deps,
  ...
  it('accepts all-kept and no-task stale resolutions idempotently', async () => {
    for (const tasks of [[makeTask({ prWorkspaceState: 'kept' })], []]) {
      const deps = makeDeps(tasks);
      await expect(
        resolveClosedPrWorkspace(
          { projectId: 'project-1', pullRequestId: 12, action: 'keep' },
  ...
  it('emits nothing when atomic keep transition rolls back', async () => {
    const tasks = [makeTask(), makeTask({ id: 'task-2' })];
    const deps = makeDeps(tasks);
    const persistedStates = new Map(
      tasks.map((task) => [task.id, task.prWorkspaceState]),
    );
    vi.mocked(deps.keepPrWorkspaces).mockImplementation(async (taskIds) => {
      const snapshot = new Map(persistedStates);
      persistedStates.set(taskIds[0], 'kept');
      persistedStates.clear();
      snapshot.forEach((state, taskId) => persistedStates.set(taskId, state));
    ...
    expect([...persistedStates.values()]).toEqual([
      'cleanup-pending',
      'cleanup-pending',
    ]);
  });

  it('continues all keep events when one emitter fails after commit', async () => {
    const deps = makeDeps([makeTask(), makeTask({ id: 'task-2' })]);
    vi.mocked(deps.emitTaskUpsert)
      .mockImplementationOnce(() => {
        throw new Error('window closed');
      })
      .mockImplementationOnce(() => undefined);
    ...
    expect(deps.keepPrWorkspaces).toHaveBeenCalledOnce();
    expect(deps.emitTaskUpsert).toHaveBeenCalledTimes(2);
  });
});
