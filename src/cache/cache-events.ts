...
/**
 * PR workspace task deletions must still reconcile renderer caches even when
 * no cache resource subscription is active, so the listener can force-apply
 * them.
 */
export function isPrWorkspaceTaskDeleteEvent(event: CacheEvent) {
  if (event.type !== 'task.delete') return false;
