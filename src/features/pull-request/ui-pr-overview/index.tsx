    ...
    <div className="border-glass-border bg-bg-1 overflow-hidden rounded-lg border">
      ...
      <div className="p-4">
        {isEditing ? <div className="space-y-3">
          ...
          {images.length > 0 && <div className="flex flex-wrap gap-1.5">{images.map((image, index) => <div key={`${image.filename ?? 'img'}-${index}`} className="relative">{imagePreviewUrls[index] ? <img src={imagePreviewUrls[index]} alt={image.filename || 'Attached image'} title={image.sizeBytes ? formatBytes(image.sizeBytes) : undefined} className="h-8 w-8 rounded border-glass-border border object-cover" /> : <div title={image.filename} className="text-ink-3 border-stroke-1 flex h-8 max-w-36 items-center rounded border px-1.5 text-[9px]"><span className="truncate">{image.filename ?? image.mimeType}</span></div>}{image.sizeBytes && <span className="absolute right-0 bottom-0 left-0 bg-scrim-strong text-chrome-fg rounded-b px-0.5 text-center font-mono text-[7px] leading-3">{formatBytes(image.sizeBytes)}</span>}<button type="button" aria-label={`Remove ${image.filename ?? 'attached image'}`} onClick={() => removeImage(index)} disabled={controlsLocked} className="absolute -top-1 -right-1 flex h-3.5 w-3.5 items-center justify-center bg-scrim-strong text-chrome-fg rounded-full opacity-60 transition-opacity hover:opacity-100 focus-visible:opacity-100 disabled:pointer-events-none disabled:opacity-30"><X className="h-2.5 w-2.5" /></button></div>)}</div>}
